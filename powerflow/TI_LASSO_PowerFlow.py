...
def generate_data(n, m):
    ...
    np.savetxt(f"data/WSG_adj_m{m}_n{n}.csv", adj_mat.todense(), delimiter=",", fmt='%d')
    np.savetxt(f"data/WSG_r_m{m}_n{n}.csv", r.todense(), delimiter=",", fmt='%.2f')
    np.savetxt(f"data/WSG_v_m{m}_n{n}.csv", v, delimiter=",", fmt='%.2f')
    np.savetxt(f"data/WSG_iflow_m{m}_n{n}.csv", Iflow, delimiter=",", fmt='%.2f')
    return adj_mat, r, v, Iflow


def generate_data_dcpf(): # 用直流潮流数据替换上面生成的试验数据

    pass


def graph_connected():
    adj = pd.read_csv(f'data/WSG_adj_m{m}_n{n}.csv', header=None)
    adj = np.matrix(adj)
...
def dataset_nodei(nodeIndex, m, n, percent):
    ...
    # v_mn = pd.read_csv(f'data/WSG_v_m{m}_n{n}.csv', header=None)
    v_mn = pd.read_csv(f'data/dcpf_69_Va.csv', header=None)  # Va的相角差决定pi
    dfx = v_mn.sub(v_mn.iloc[:, nodeIndex], axis=0) * -1
    dfx.drop(columns=[nodeIndex], axis=1, inplace=True)
    # i_mn = pd.read_csv(f'data/WSG_iflow_m{m}_n{n}.csv', header=None)
    i_mn = pd.read_csv(f'data/dcpf_69_P.csv', header=None)  # 直流潮流中的pi 相当于之前的电流
...
def volt_correlation(nodeIndex,m,n):
    # v_mn = pd.read_csv(f'data/WSG_v_m{m}_n{n}.csv', header=None)
    v_mn = pd.read_csv(f'data/dcpf_69_Va.csv', header=None)
    dfx = v_mn.sub(v_mn.iloc[:, nodeIndex], axis=0) * -1
    print(dfx)
    dfx.drop(columns=[nodeIndex], axis=1, inplace=True)
    print(dfx)
...
def classify_result(pct, m, n):
    ...
    for node in range(n):
        ...
        # adj = pd.read_csv(f'data/WSG_adj_m{m}_n{n}.csv', header=None)
        adj = pd.read_csv(f'data/69_adj.csv', header=None)
...
#adj, r, v, iflow = generate_data(n, m)
